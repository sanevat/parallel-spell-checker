#!/usr/bin/env python3
"""
Generate 1000-word test files and run full benchmark (Sequential + MPI).
"""
...
def generate_test_files(num_typos: int = 1000):
    ...
    for lang, valid_words, dict_set, by_len, chars in [
        ...
        # Save files
        typo_file = BASE / f"data/test_texts/{'macedonian' if lang == 'mk' else 'english'}/{lang}_hunspell_typos_1K.txt"
        gt_file = BASE / f"data/ground_truth/{lang}_hunspell_corrections_1K.json"
...
def main():
    ...
    gen_results = generate_test_files(1000)
    ...
    # Load ground truth
    with open(BASE / "data/ground_truth/mk_hunspell_corrections_1K.json", 'r', encoding='utf-8') as f:
        gt_mk = json.load(f)
    with open(BASE / "data/ground_truth/en_hunspell_corrections_1K.json", 'r', encoding='utf-8') as f:
        gt_en = json.load(f)
    ...
    all_results = {
        'description': 'Scalability benchmark with 1000 typos, 50K dict',
        'generation': {
    ...
    results_file = BASE / "results" / "scalability_1K.json"
    ...
    for num_procs in [2, 4, 8]:
        ...
        mpi_script = BASE / "spell_checker" / "mpi_benchmark_1K.py"
        cmd = ["mpiexec", "-n", str(num_procs), "python", "-u", str(mpi_script)]

        subprocess.run(cmd, cwd=str(BASE))

        # Read results
        temp_file = BASE / "results" / "mpi_1K_temp.json"
        if temp_file.exists():
    ...
    print("\n" + "=" * 100)
    print("FINAL COMPARISON TABLE (1000 words)")
    print("=" * 100)
