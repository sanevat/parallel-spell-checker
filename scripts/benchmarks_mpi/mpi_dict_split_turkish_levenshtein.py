#!/usr/bin/env python3
"""
MPI Dict-Split Spell Checker for Turkish - Levenshtein.
...
def run_benchmark(comm, typos, dict_by_len, ground_truth):
    ...
    if rank == 0:
        print(f"\n--- [DICT-SPLIT MPI-{size}] Levenshtein + TR ({len(typos)} words) ---")
        sys.stdout.flush()
...
def main():
    ...
    if rank == 0:
        print("=" * 80)
        print(f"DICT-SPLIT MPI-{size} BENCHMARK - TURKISH LEVENSHTEIN ({file_size})")
        print("=" * 80)
    ...
    if rank == 0 and result:
        ...
        temp_file = results_dir / f"tr_levenshtein_dict_split_{file_size}_temp.json"
