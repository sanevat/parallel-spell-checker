#!/usr/bin/env python3
"""
Run TEXT-SPLIT MPI benchmarks_mpi for 250 test files.
...
def main():
    print("=" * 100)
    print("TEXT-SPLIT MPI BENCHMARK (250 words)")
    print("=" * 100)
    print("\nRunning TEXT-SPLIT MPI-2, MPI-4, MPI-8")
    sys.stdout.flush()

    all_results = {
        'description': 'TEXT-SPLIT benchmark for 250 words',
        'text_split': {}
    }
    ...
    for num_procs in [2, 4, 8]:
        ...
        cmd = ["mpiexec", "-n", str(num_procs), "python", "-u",
               str(BASE / "spell_checker" / "mpi_text_split_250.py")]
        subprocess.run(cmd, cwd=str(BASE))

        temp_file = BASE / "results" / "text_split_250_temp.json"
        if temp_file.exists():
    ...
    # Print final table
    print("\n" + "=" * 100)
    print("FINAL TEXT-SPLIT RESULTS (250 words)")
    print("=" * 100)
    ...
    # Save results
    results_file = BASE / "results" / "text_split_250.json"
