#!/usr/bin/env python3
"""
MPI Dict-Split Benchmark for Damerau-Levenshtein.
...
def run_benchmark(comm, typos, dict_by_len, ground_truth, lang):
    ...
    if rank == 0:
        print(f"\n--- [MPI-{size}] Damerau-Levenshtein + {lang} ({len(typos)} words) ---")
        sys.stdout.flush()
...
def main():
    ...
    # Get dataset size from environment variable (default 250)
    dataset_size = os.environ.get('DATASET_SIZE', '250')

    # Map size labels to file suffixes
    size_suffix = dataset_size
    ...
    if rank == 0:
        print(f"\n--- MPI-{size} Dict-Split Damerau-Levenshtein Benchmark (Dataset: {dataset_size}) ---")
        ...
        gt_mk = load_ground_truth(str(base / f"data/ground_truth/mk_hunspell_corrections_{size_suffix}.json"))
        gt_en = load_ground_truth(str(base / f"data/ground_truth/en_hunspell_corrections_{size_suffix}.json"))
    ...
    if rank == 0:
        ...
        output_file = results_dir / "dict_split_temp.json"
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump({"num_procs": size, "dataset_size": dataset_size, "results": results}, f, indent=2, ensure_ascii=False)
