#!/usr/bin/env python3
"""
MPI Dict-Split Spell Checker for Turkish - Myers Bit-Vector.
...
def run_benchmark(comm, typos, dict_by_len, ground_truth):
    ...
    if rank == 0:
        print(f"\n--- [DICT-SPLIT MPI-{size}] Myers Bit-Vector + TR ({len(typos)} words) ---")
        sys.stdout.flush()
...
def main():
    ...
    if rank == 0:
        print("=" * 80)
        print(f"DICT-SPLIT MPI-{size} BENCHMARK - TURKISH MYERS BIT-VECTOR ({file_size})")
        print("=" * 80)
    ...
    if rank == 0 and result:
        ...
        temp_file = results_dir / f"tr_myers_dict_split_{file_size}_temp.json"
