#!/usr/bin/env python3
"""
MPI Text-Split Spell Checker for Turkish - Damerau-Levenshtein.
...
def run_text_split_benchmark(comm, typos, by_len, ground_truth):
    ...
    if rank == 0:
        print(f"\n--- [TEXT-SPLIT MPI-{size}] Damerau-Levenshtein + TR ({len(typos)} words) ---")
        sys.stdout.flush()
    ...
    if rank == 0:
        ...
        return TextSplitResult(
            algorithm="Damerau-Levenshtein", language="TR", num_procs=size, misspelled=n,
...
def main():
    ...
    if rank == 0:
        print("=" * 80)
        print(f"TEXT-SPLIT MPI-{size} BENCHMARK - TURKISH DAMERAU-LEVENSHTEIN ({file_size})")
        print("=" * 80)
    ...
    if rank == 0 and result:
        ...
        temp_file = results_dir / f"tr_damerau_text_split_{file_size}_temp.json"
