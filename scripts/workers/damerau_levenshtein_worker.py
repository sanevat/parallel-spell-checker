#!/usr/bin/env python3
"""
MPI worker for Damerau-Levenshtein on specified dataset size.
Usage: mpiexec -n N python damerau_levenshtein_worker.py <dataset_size>
"""
...
def run_benchmark(comm, typos, by_len, ground_truth, lang):
    ...
    local_results = process_chunk(local_typos, by_len, damerau_levenshtein)
    comm.Barrier()
    ...
    if rank == 0:
        ...
        return Result(
            algorithm="Damerau-Levenshtein", language=lang, num_procs=size, misspelled=n,
...
def main():
    if len(sys.argv) < 2:
        print("Usage: mpiexec -n N python damerau_levenshtein_worker.py <dataset_size>")
        sys.exit(1)
    ...
    if rank == 0:
        ...
        with open(results_dir / "damerau_levenshtein_temp.json", 'w', encoding='utf-8') as f:
