#!/usr/bin/env python3
"""
MPI worker for Myers Bit-Vector on specified dataset size.
Usage: mpiexec -n N python myers_worker.py <dataset_size>
"""
...
def run_benchmark(comm, typos, by_len, ground_truth, lang):
    ...
    if rank == 0:
        ...
        return Result(
            algorithm="Myers Bit-Vector", language=lang, num_procs=size, misspelled=n,
...
def main():
    if len(sys.argv) < 2:
        print("Usage: mpiexec -n N python myers_worker.py <dataset_size>")
        sys.exit(1)
    ...
    if rank == 0:
        ...
        with open(results_dir / "myers_temp.json", 'w', encoding='utf-8') as f:
