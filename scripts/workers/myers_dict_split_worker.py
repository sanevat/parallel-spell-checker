#!/usr/bin/env python3
"""
MPI worker for Dict-Split Myers Bit-Vector on specified dataset size.
Strategy: Split CANDIDATES across processes for each typo word.
Usage: mpiexec -n N python myers_dict_split_worker.py <dataset_size>
"""
...
from spell_checker.algorithms.myers_bitvector import myers_bitvector
...
def find_local_best(typo: str, candidates: List[str]) -> Tuple[str, int]:
    ...
    best_dist = myers_bitvector(typo, candidates[0])

    for c in candidates[1:]:
        d = myers_bitvector(typo, c)
        if d < best_dist:
            best_dist = d
            best_word = c
            if d == 0:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: mpiexec -n N python myers_dict_split_worker.py <dataset_size>")
        sys.exit(1)
    ...
    if rank == 0:
        ...
        with open(results_dir / "myers_dict_split_temp.json", 'w', encoding='utf-8') as f:
