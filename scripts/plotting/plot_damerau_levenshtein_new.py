"""
Plot Damerau-Levenshtein Scalability - New Results

Visualizes the text-split benchmark results for Damerau-Levenshtein algorithm
    ...
    for num_procs in ['1', '2', '4', '8']:
        if num_procs in text_split:
            for entry in text_split[num_procs]:
                if entry['algorithm'] == 'Damerau-Levenshtein':
                    lang = entry['language']
                    time_s = entry['total_time_s']
                    data[int(num_procs)][lang].append(time_s)

# Load Turkish data from separate results file
tr_results_path = results_dir / 'results' / 'text_split' / 'tr_damerau_best_results.json'
...
ax.set_title('MPI Text-Split Scalability: Damerau-Levenshtein Algorithm')
...
plt.savefig(output_dir / 'damerau_levenshtein_scalability_new.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig(output_dir / 'damerau_levenshtein_scalability_new.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')

# Also save in main directory
plt.savefig(results_dir / 'damerau_levenshtein_scalability_new.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')

print(f"Saved visualizations:")
print(f"  - {output_dir / 'damerau_levenshtein_scalability_new.png'}")
print(f"  - {output_dir / 'damerau_levenshtein_scalability_new.pdf'}")
print(f"  - {results_dir / 'damerau_levenshtein_scalability_new.png'}")

# Print summary statistics
print("\n--- Damerau-Levenshtein Performance Summary ---")
