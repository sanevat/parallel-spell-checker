...
def load_mk_en_data(algorithm_names):
    """Load MK and EN data from dict_split files."""
    ...
    for file_path, num_words in file_info:
        ...
        for num_procs_str in ['1', '2', '4', '8']:
            ...
            if num_procs_str in dict_split:
                for entry in dict_split[num_procs_str]:
...
def load_tr_data(tr_file, algorithm_names):
    ...
    if not os.path.exists(tr_path):
        print(f"Warning: TR results file not found: {tr_path}")
        return data
...
def create_algorithm_plot(algorithm_key, algorithm_names, tr_file, output_name, title):
    ...
    # Main title
    fig.suptitle(f'MPI Dict-Split Scalability: {title}', fontsize=14, fontweight='bold')
...
# Generate plots for Myers and Levenshtein dict-split
algorithms = [
    ('Myers-BitVector', ['Myers Bit-Vector', 'Myers-BitVector'],
     'results/dict_split/tr_myers_dict_split_best_results.json',
     'dict_split_myers_bitvector', 'Myers Bit-Vector Algorithm'),

    ('Levenshtein', ['Levenshtein'],
     'results/dict_split/tr_levenshtein_dict_split_best_results.json',
     'dict_split_levenshtein', 'Levenshtein Algorithm'),
]
