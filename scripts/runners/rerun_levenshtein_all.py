#!/usr/bin/env python3
"""
Re-run Levenshtein for 1, 2, 4, 8 procs on all datasets, 2 times each.
...
def run_levenshtein(num_procs, dataset_size):
    """Run Levenshtein benchmark and return results."""
    cmd = ["mpiexec", "-n", str(num_procs), "python", "-u",
           str(BASE / "scripts" / "levenshtein_worker.py"), dataset_size]
    subprocess.run(cmd, cwd=str(BASE))

    temp_file = BASE / "results" / "levenshtein_temp.json"
    if temp_file.exists():
...
def main():
    print("=" * 80)
    print("Levenshtein Re-run (all datasets)")
    ...
    for dataset_size, json_file in DATASETS:
        ...
        for run in range(1, 3):
            ...
            for num_procs in [1, 2, 4, 8]:
                ...
                results = run_levenshtein(num_procs, dataset_size)
        ...
        for num_procs in [1, 2, 4, 8]:
            ...
            for best in best_results[num_procs]:
                ...
                for i, entry in enumerate(all_data['text_split'][procs_key]):
                    if entry['algorithm'] == 'Levenshtein' and entry['language'] == best['language']:
