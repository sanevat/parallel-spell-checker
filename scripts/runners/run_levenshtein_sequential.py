#!/usr/bin/env python3
"""
Run Levenshtein Sequential (1 process) Benchmark

Runs Levenshtein algorithm sequentially for all word counts
...
def run_sequential_benchmark(typos: List[str], by_len: Dict[int, List[str]],
    ...
    """Run Levenshtein sequentially on all typos."""
    ...
    for i, typo in enumerate(typos):
        best_word, num_cand = find_best_correction(typo, by_len, levenshtein)
...
def run_for_word_count(base: Path, word_count: int, suffix: str) -> List[SequentialResult]:
    ...
    print(f"\n{'='*80}")
    print(f"RUNNING SEQUENTIAL LEVENSHTEIN FOR {word_count} WORDS")
    print(f"{'='*80}")
...
def update_json_file(json_path: Path, results: List[SequentialResult]):
    ...
    if '1' not in data['text_split']:
        data['text_split']['1'] = []

    # Update or add Levenshtein results
    for result in results:
        result_dict = asdict(result)
        found = False
        for i, entry in enumerate(data['text_split']['1']):
            if entry['algorithm'] == 'Levenshtein' and entry['language'] == result.language:
                data['text_split']['1'][i] = result_dict
                found = True
                break

        if not found:
            data['text_split']['1'].append(result_dict)
...
def main():
    print("=" * 80)
    print("LEVENSHTEIN SEQUENTIAL BENCHMARK")
    ...
    # Print final summary
    print("\n" + "=" * 80)
    print("FINAL SUMMARY - LEVENSHTEIN SEQUENTIAL (1 process)")
    print("=" * 80)
    ...
    print("=" * 80)
    print("Done! All JSON files updated with 1-process Levenshtein results.")
